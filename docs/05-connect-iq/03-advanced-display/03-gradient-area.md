...
https://forums.garmin.com/developer/connect-iq/f/discussion/359519/gradient-color-and-transparency-on-amoleds#pifragment-1298=1
...
```c
		var startColor = Gfx.COLOR_RED;
		var midColor = Gfx.COLOR_GREEN;
		var endColor = Gfx.COLOR_BLUE;
		var startX = 100;
		var startY = 100;
		var endX = 300;
		var endY = 150;
...
class ColorUtils {
	...
	static function drawGradient(dc, startColor, midColor, endColor, startX, startY, endX, endY) {
		if (midColor < 0) {
        	...
        	// dc.drawLine(startY, endY, endX, endY);
	...
	static function lerpCol0orsCompact(startColor as Number, endColor as Number, ratio as Float) {		
		var mask1 = 0xff00ff;
	...
	// 绘制与左上到右下对角线垂直的渐变斜线
	static function drawGradientLURD(dc, startColor, midColor, endColor, x1, y1, x2, y2) {
		var width = x2 - x1;
		var height = y2 - y1;
		
		var angleTan = height.toFloat() / width.toFloat();

		// 计算需要绘制的线条数量
		var lineCount = (width + height.toFloat() * angleTan).toNumber();
		for (var i = 0; i < lineCount; i ++) {
			var ratio = i.toFloat() / lineCount;
			var color = getLerpedColor(startColor, midColor, endColor, ratio);
			dc.setColor(color, Graphics.COLOR_TRANSPARENT);

			// 计算线条的起点
			var startX, startY, endX, endY;
			if (i < height * angleTan) {
				startX = x1 + i;
				startY = y1;
				endX = x1 ;
				endY = y1 + i / angleTan;
				...
				startX = x1 + i;
				startY = y1;
				endX = x1 + i - height * angleTan;
				endY = y2;
			} else { // i > width && i <= width + height
				startX = x2;
				startY = y1 + (i - width) / angleTan;
				endX = x1 + i - height * angleTan;
				endY = y2;
			}
			System.println("startX: " + startX + " startY: " + startY + " endX: " + endX + " endY: " + endY);
			dc.drawLine(startX.toNumber(), startY.toNumber(), endX.toNumber(), endY.toNumber());
